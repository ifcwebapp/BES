...
{
    public class OtherResourcesController : BaseController
    {
        public IHttpActionResult GetOtherResources(string countryCode)
        {
            return this.WatchForErrors(
                () => new Repository().GetOtherResources(countryCode),
                "Unable to get other resources."
            );
        }
    }
}
