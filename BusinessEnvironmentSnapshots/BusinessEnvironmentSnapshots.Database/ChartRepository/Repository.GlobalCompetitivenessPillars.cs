...
namespace BusinessEnvironmentSnapshots.Database.ChartRepository
{
    ...
    {
        private SingleChart GetGlobalCompetitivenessPillarsChart(string id, string countryCode, string yearConfig)
        {
            ...
            if (chartInfo != null)
            {
                ...
                ConvertToSeries(cleanedData,
                    ...
                    SeriesType.Polar,
                    countries.ToDictionary(k => k.Key, k => k.Value.Name), g => g.IndicatorId, d => new AdditionalDataPoint { Value = d },
                    out cats, out sers);
                    ...
                    for (int i = 0; i < serie.Values.Count; i++)
                    {
                        ...
                        if (
                            dataDictionary.TryGetValue(
                                new Tuple<int, int, int>(serie.Id, supportedIndicators[new Tuple<int, int>(indicatorIds[i], 1)].SupportedIndicatorId, years.First()),
                                out rank))
                        {
