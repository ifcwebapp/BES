...
namespace BusinessEnvironmentSnapshots.Database.ChartRepository
{
    ...
    {
        private SingleChart GetReformsValidatedByDoingBusinessChart(string id, string countryCode, string yearConfig)
        {
            ...
            if (chartInfo != null)
            {
                ...
                ConvertToSeries(cleanedData,
                    ...
                    out cats, out sers);
                var serie = sers.FirstOrDefault(s => s.Id != requestedCountryId);
                if (serie != null)
                {
                    serie.SeriesType = SeriesType.Line;
                    //serie.Name = chartInfo.ChartYaxisTitle;
                    //serie.SerieCountryType = getSerieCountryType(serie.Id, countries);
                }
                
                sers.ForEach(s => s.Name = chartInfo.ChartYaxisTitle);
