...
namespace BusinessEnvironmentSnapshots.Database.ChartRepository
{
    ...
    {
        private SingleChart GetPublicAndPrivateCreditCoverageChart(string id, string countryCode, string yearConfig)
        {
            ...
            if (chartInfo != null)
            {
                ...
                ConvertToSeries(cleanedData,
                    data.Select(d => d.CountryId)
                    .Distinct().OrderBy(cId => countries[cId].CountryType).ToList(),
